...
- **God file decomposition**: `orchestrator.py`, `predicate.py`, and `vertex.py`
  safely decomposed into packages using the Facade pattern for API compatibility.

### Added

- **7 regression tests** in `tests/regression/test_sdlc_gate_defects.py` with
  discrimination guards (POSIX path mismatch, numbering uniqueness, toolchain lockstep).
- **Facade identity contract** for `core.orchestrator`, `workflow.predicate`, and
  `adapters.llm.vertex` registered in `tests/test_import_compat.py`.
- **Clock seam** (`mangomas.utils.clock.now`) for deterministic time in tests.
- **3 AI skills**: `mango-composition-builder`, `mango-workflow-generator`,
  `mango-eval-runner` in `.claude/skills/`.
...
[Unreleased]: https://github.com/Mango-Metrics-NLM/MangoMas_V2/compare/v0.4.0...HEAD
