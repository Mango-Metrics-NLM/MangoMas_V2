...
async def test_jsonl_source_matches_load_jsonl(fixtures_dir: Path) -> None:
    ...
    via_source = await JsonlSource(path=str(path)).load()
...
async def test_inline_source_loads_and_validates() -> None:
    source = InlineSource(rows=[_INLINE_ROW])
...
async def test_inline_source_rejects_bad_row() -> None:
    ...
    source = InlineSource(rows=[{"messages": [{"role": "user", "content": "x"}]}])
...
def test_sources_satisfy_protocol() -> None:
    assert isinstance(JsonlSource(path="x"), DatasetSource)
    assert isinstance(InlineSource(rows=[]), DatasetSource)
