...
| 1.5 | **`MODEL_OVERRIDE` wiring**: a composition-layer change per spec-0014 R4 (per-agent `LLMClient` rather than one shared `ctx.llm`), explicitly *not* protected-path work. ✅ **Landed 2026-09-01** (spec-0028 / ADR-0028) — confirmed *not* protected-path work as predicted here, via a composition-layer `Orchestrator._close_hooks()` extension rather than editing `core/orchestrator.py`. `ctx.memory` — fully wired, closed correctly, and read by nothing **[On trust]** — is a feature bet, not a bug: decision D4c. | P1 | |
...
| D4 | Retire-vs-fix calls: **(a)** `MIN_CHUNK_WORDS` — doc fix (see §6.1); retiring the knob would break the config contract tests and is not mechanical. **(b)** `LoopSettings` / `MODEL_OVERRIDE` — wire (1.2b / 1.5). ✅ Both wired (`LoopSettings` via spec-0026, `MODEL_OVERRIDE` via spec-0028). **(c)** `ctx.memory` — keep-as-experimental vs fund a consuming agent vs remove. | (a) doc fix; (b) wire; (c) keep-experimental unless a consumer is funded. |
...
Exact CHANGELOG size; `ctx.memory` consumer
absence (`MODEL_OVERRIDE` gained a consumer 2026-09-01, spec-0028); base-image pinning mode; stale `observability.md`/`cloud-providers.md`
