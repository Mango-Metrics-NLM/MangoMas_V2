"""``ConfigChange`` hook decision logic (ADR-0021 / spec-0017).

Ported from ``origin/main``'s ``harness/config_audit.py`` (ADR-0011 there) —
this module is pure decision logic, unaffected by the branch's differing
protected-path enforcement design, so it ports without adaptation.
...
a git diff (this repo's ``PreToolUse`` protected-path hook already covers
that ground, advisory-only per ADR-0021 — a ``ConfigChange``-time staged-diff
read would not reflect an in-session config change anyway).
"""
...
def evaluate_config_change(source: str | None, mode: ConfigChangeMode) -> ConfigAuditDecision:
    ...
    - *source* is ``None``: fails open (``allow``) — the stdin field naming
      the changed source is undocumented upstream; an absent or unrecognized
      value must never silently escalate to a block.
