...
paths** (`agent.py`, `orchestrator/`, `structured.py`, `tools.py`;
...
| `orchestrator/` | **yes** | `dispatch`, `dispatch_pipeline`, `dispatch_fan_out`, `stream_dispatch` |
...
| Surface | Agent |
|---|---|
| `orchestrator/` and the whole dispatch surface | `mango-orchestrator-dev` |
...
- `core/` imports only `mangomas.errors`, `mangomas.registry` and — since
  ...
  `orchestrator/` already imports; the record helpers are no-ops until
